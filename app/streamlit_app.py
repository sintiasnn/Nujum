...
with st.sidebar:
    ...
    periods = st.selectbox(
        "Prediksi untuk berapa hari?",
        options=[7, 14, 30, 60, 90],
        index=2,
        help="Pilih jangka waktu prediksi ke depan"
    )
    ...
    safety_factor = st.slider(
        ...
        value=1.2,
        step=0.05,
        help="Faktor pengali untuk menghitung stok cadangan. 1.2 = tambah 20% dari prediksi."
    )

    st.divider()
    st.markdown("### 📖 Panduan")
    st.markdown("""
    1. **Upload** file CSV data penjualan
    2. **Pilih kolom** tanggal, jumlah, & produk
    3. Klik **Mulai Training**
    4. Pilih produk & lihat **prediksi + rekomendasi stok**
    """)
    st.divider()
    st.markdown("*Dibuat untuk IDCamp Developer Challenge 2026* 🇮🇩")


# ── Main Content ────────────────────────────────────────────────────────────────

st.markdown('<p class="main-title">🔮 Nujum</p>', unsafe_allow_html=True)
st.markdown('<p class="tagline">Ramal penjualanmu, kelola stok lebih cerdas — untuk UMKM Indonesia</p>', unsafe_allow_html=True)

tab1, tab2, tab3 = st.tabs(["📤 Upload & Training", "🔮 Prediksi & Stok", "📊 Analitik"])
...
with tab1:
    ...
    st.markdown("Upload file CSV berisi data penjualan historis tokomu. Minimal 30 hari data.")
...
with tab2:
    ...
    if not available_models:
        st.warning("⚠️ Belum ada model yang ditraining. Silakan upload data dan lakukan training di tab **📤 Upload & Training** terlebih dahulu.")
    else:
        ...
        if "recommendation" in st.session_state:
            ...
            <div class="recommendation-box">
                <h3>📦 Rekomendasi Stok untuk {periods} Hari ke Depan</h3>
                <h1 style="font-size: 3rem; margin: 0.5rem 0;">{rec['recommended_stock']:,} unit</h1>
                <p style="opacity: 0.85;">Termasuk buffer keamanan {int((safety_factor-1)*100)}% dari prediksi dasar</p>
            </div>
            """, unsafe_allow_html=True)
...
with tab3:
    ...
    available_models = get_available_models()
    if not available_models:
        st.warning("⚠️ Belum ada model yang tersedia. Lakukan training terlebih dahulu.")
    else:
